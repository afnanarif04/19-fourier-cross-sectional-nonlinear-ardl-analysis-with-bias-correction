# =============================================================================
# 16_mc_fcsnardl_3_HPJ.R
# FCS-NARDL Monte Carlo - Half-Panel Jackknife
#
...
# Output   : 16_mc_fcsnardl_3_HPJ_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" FCS-NARDL  |  Script 3 of 6  |  METHOD: HPJ\n")
cat("================================================================\n\n")
...
EstHPJ_FCS <- function(Y, X, N, TT, PT, k_fourier=1) {
  T1 <- floor(TT/2); T2 <- TT-T1
  if (T1 < PT+2 || T2 < PT+2) return(EstFCS_NARDL(Y,X,N,TT,PT,k_fourier))
  full <- EstFCS_NARDL(Y,X,N,TT,PT,k_fourier)
  h1 <- EstFCS_NARDL(Y[,1:T1,drop=F],X[,1:T1,drop=F],N,T1,PT,k_fourier)
  h2 <- EstFCS_NARDL(Y[,(T1+1):TT,drop=F],X[,(T1+1):TT,drop=F],N,T2,PT,k_fourier)
...
for (g in 1:G) {
  ...
  for (r in 1:R) {
    dat <- GenerateDGP_FCS(N,TT,m,rho_f,rho_x,sig_f,sig_u,sig_v,phi_true,theta_pos_true,theta_neg_true,
                            alpha1_true,alpha2_true,k_fourier,gamma_y,gamma_x)
    res <- EstHPJ_FCS(dat$Y,dat$X,N,TT,PT,k_fourier)
  ...
  results <- rbind(results, data.frame(
    N=N,T=TT, HPJ_phi_bias=round(mean(phi_hat)-phi_bar,4),
...
write.csv(results,"16_mc_fcsnardl_3_HPJ_results.csv",row.names=FALSE)
cat("\n=== DONE.  Saved: 16_mc_fcsnardl_3_HPJ_results.csv ===\n")
