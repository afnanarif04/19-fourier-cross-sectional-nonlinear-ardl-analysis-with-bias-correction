# =============================================================================
# 15_mc_fcsnardl_2_RMA.R
# FCS-NARDL Monte Carlo - Recursive Mean Adjustment
#
...
# Output   : 15_mc_fcsnardl_2_RMA_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" FCS-NARDL  |  Script 2 of 6  |  METHOD: RMA\n")
cat("================================================================\n\n")

set.seed(2026)

EstRMA_FCS <- function(Y, X, N, TT, PT, k_fourier = 1) {
  nobs <- (TT-1)-PT
  ...
  for (i in 1:N) {
    ...
    for (tt in (PT+2):TT) {
      ...
      for (ell in 0:PT) {
        s <- tt-ell
        YB[row,ell+1] <- mean(Y[,1:s]); XB[row,ell+1] <- mean(X[,1:s])
...
GenerateDGP_FCS <- function(N,TT,m,rho_f,rho_x,sig_f,sig_u,sig_v,
  ...
  for (i in 1:N) for (tt in 2:TT_total) {
    ...
    Y[i,tt] <- rho_i*Y[i,tt-1]+
      phi_true[i]*(-theta_pos_true[i])*pmax(X[i,tt-1]-X[i,1],0)+
      phi_true[i]*(-theta_neg_true[i])*pmin(X[i,tt-1]-X[i,1],0)+
      0.3*(X[i,tt]-X[i,tt-1])+
      alpha1_true[i]*sin(2*pi*k_fourier*tt/TT_total)+
...
for (g in 1:G) {
  ...
  for (r in 1:R) {
    dat <- GenerateDGP_FCS(N,TT,m,rho_f,rho_x,sig_f,sig_u,sig_v,
                            phi_true,theta_pos_true,theta_neg_true,
                            alpha1_true,alpha2_true,k_fourier,gamma_y,gamma_x)
    res <- EstRMA_FCS(dat$Y,dat$X,N,TT,PT,k_fourier)
  ...
  results <- rbind(results, data.frame(
    N=N,T=TT,
    RMA_phi_bias=round(mean(phi_hat)-phi_bar,4),
    RMA_phi_rmse=round(sqrt(mean((phi_hat-phi_bar)^2)),4),
    RMA_phi_pctbias=round(100*(mean(phi_hat)-phi_bar)/abs(phi_bar),2),
    RMA_thpos_bias=round(mean(thp_hat)-theta_pos_true_val,4),
    RMA_thpos_rmse=round(sqrt(mean((thp_hat-theta_pos_true_val)^2)),4),
    RMA_thneg_bias=round(mean(thn_hat)-theta_neg_true_val,4),
    RMA_thneg_rmse=round(sqrt(mean((thn_hat-theta_neg_true_val)^2)),4)
  ))
  cat(sprintf("phi bias=%.4f\n",mean(phi_hat)-phi_bar))
}
write.csv(results,"15_mc_fcsnardl_2_RMA_results.csv",row.names=FALSE)
cat("\n=== DONE.  Saved: 15_mc_fcsnardl_2_RMA_results.csv ===\n")
