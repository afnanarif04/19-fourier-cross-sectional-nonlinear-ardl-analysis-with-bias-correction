# =============================================================================
# 11_mc_csnardl_4_TPJ.R
# CS-NARDL Monte Carlo - Third-Panel Jackknife
#
...
# Output   : 11_mc_csnardl_4_TPJ_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" CS-NARDL  |  Script 4 of 6  |  METHOD: TPJ\n")
cat("================================================================\n\n")
...
EstTPJ_NARDL <- function(Y, X, N, TT, PT) {
  T3a <- floor(TT/3); T3c <- TT-2*T3a
  if (T3a < PT+2 || T3c < PT+2) return(EstCS_NARDL(Y,X,N,TT,PT))
  full <- EstCS_NARDL(Y,X,N,TT,PT)
  e12 <- 1:(2*T3a); T12 <- length(e12)
  e23 <- (T3a+1):TT; T23 <- length(e23)
  e13 <- c(1:T3a, (2*T3a+1):TT); T13 <- length(e13)
  r12 <- EstCS_NARDL(Y[,e12,drop=F],X[,e12,drop=F],N,T12,PT)
  r23 <- EstCS_NARDL(Y[,e23,drop=F],X[,e23,drop=F],N,T23,PT)
  r13 <- EstCS_NARDL(Y[,e13,drop=F],X[,e13,drop=F],N,T13,PT)
...
for (g in 1:G) {
  ...
  for (r in 1:R) {
    ...
    res <- EstTPJ_NARDL(dat$Y,dat$X,N,TT,PT)
  ...
  results <- rbind(results, data.frame(
    N=N,T=TT,
    TPJ_phi_bias=round(mean(phi_hat)-phi_bar,4),
...
write.csv(results,"11_mc_csnardl_4_TPJ_results.csv",row.names=FALSE)
cat("\n=== DONE.  Saved: 11_mc_csnardl_4_TPJ_results.csv ===\n")
