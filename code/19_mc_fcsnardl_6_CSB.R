# =============================================================================
# 19_mc_fcsnardl_6_CSB.R
# FCS-NARDL Monte Carlo - Cross-Section Bootstrap
#
...
# Output   : 19_mc_fcsnardl_6_CSB_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" FCS-NARDL  |  Script 6 of 6  |  METHOD: CSB\n")
cat("================================================================\n\n")
...
CSB_FCS <- function(Y, X, N, TT, PT, k_fourier=1, B_boot=200) {
  ...
  for (b in 1:B_boot) {
    ...
    res_b <- tryCatch(EstFCS_NARDL(Y[idx,,drop=F],X[idx,,drop=F],N,TT,PT,k_fourier),error=function(e)NULL)
    if (!is.null(res_b)) {
...
write.csv(results,"19_mc_fcsnardl_6_CSB_results.csv",row.names=FALSE)
cat("\n=== DONE.  Saved: 19_mc_fcsnardl_6_CSB_results.csv ===\n")
