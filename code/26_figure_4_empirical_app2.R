...
# Figure 4 - Country-level FCS-NARDL long-run elasticities, Application 2
#
# Two-panel line-and-dot figure. Each coloured line tracks one country's
# unit-level long-run income elasticity of CO2 across the three correctors
# (CS, RMA, HPJ). The thick black dashed line is the panel mean. Panel (a) is
# the positive partial sum (income rises); panel (b) is the negative partial
# sum (income falls). The reshuffling of unit estimates under RMA in panel (b)
# is the mechanism behind the panel-mean shift discussed in the manuscript.
#
# Input    : ../results/empirical/02_App2_estimates.rds
# Output   : ../figures/figure_4_empirical_app2.png
# Packages : base R only
#
# Run order: see 00_RUN_ALL.R (run after 22_empirical_application_2.R, or use
...
est <- readRDS("../results/empirical/02_App2_estimates.rds")
...
png("../figures/figure_4_empirical_app2.png",
...
panels <- list(
  list(field = "unit_LR_pos", mean = "LR_pos_mean",
       ylab = expression("Long-run income elasticity " * beta^"+"),
       main = "(a) Income-rise effect on CO2, by country"),
  list(field = "unit_LR_neg", mean = "LR_neg_mean",
       ylab = expression("Long-run income elasticity " * beta^"-"),
       main = "(b) Income-fall effect on CO2, by country")
)
...
for (pn in panels) {
  ...
  iso <- names(est$F_CS[[pn$field]])
  leg <- if (length(iso) >= max(pick) && !all(iso == iso[1]))
            iso[pick] else paste0("unit ", pick)
  legend("topright", legend = c(leg, "Panel mean"),
...
mtext("Figure 4. Environmental Kuznets Curve Application: country-level FCS-NARDL long-run elasticities under three correctors",
      outer = TRUE, line = 0.4, cex = 0.90, font = 2)
dev.off()
cat("Saved: ../figures/figure_4_empirical_app2.png\n")
