# =============================================================================
# 14_mc_fcsnardl_1_CS.R
# FCS-NARDL Monte Carlo - Uncorrected CCEMG (baseline)
#
# Method   : CS: Uncorrected common correlated effects mean group
# Reference : Pesaran (2006); Chudik and Pesaran (2015); Shin et al. (2014); Enders and Lee (2012)
#
...
# Output   : 14_mc_fcsnardl_1_CS_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" FCS-NARDL  |  Script 1 of 6  |  METHOD: Baseline\n")
cat("================================================================\n\n")

set.seed(2026)

# ============================================================
# 1.  FCS-NARDL ESTIMATOR
# ============================================================
EstFCS_NARDL <- function(Y, X, N, TT, PT, k_fourier = 1) {
  ybar <- colMeans(Y); xbar <- colMeans(X); dxbar <- c(0, diff(xbar))
  nobs <- (TT - 1) - PT
  phi_i <- bp_i <- bn_i <- bp1_i <- bn1_i <- a1_i <- a2_i <- numeric(N)

  for (i in 1:N) {
    yi <- Y[i, ]; xi <- X[i, ]
    DX_raw <- c(0, diff(xi))
    X_pos <- cumsum(pmax(DX_raw, 0)); X_neg <- cumsum(pmin(DX_raw, 0))

    DY <- YLAG <- XPOS <- XNEG <- XPLAG <- XNLAG <- DX <- numeric(nobs)
    SIN_T <- COS_T <- numeric(nobs)
    ...
    for (tt in (PT + 2):TT) {
      ...
      SIN_T[row] <- sin(2 * pi * k_fourier * tt / TT)
      COS_T[row] <- cos(2 * pi * k_fourier * tt / TT)
      for (ell in 0:PT) {
    ...
    a1_i[i]  <- b[7]; a2_i[i] <- b[8]  # Fourier coefficients
  }
  list(phi=phi_i, bp=bp_i, bn=bn_i, bp1=bp1_i, bn1=bn1_i, a1=a1_i, a2=a2_i)
}

# ============================================================
# 2.  DGP (FCS-NARDL: asymmetric + Fourier structural break)
# ============================================================
GenerateDGP_FCS <- function(N, TT, m, rho_f, rho_x, sig_f, sig_u, sig_v,
                             phi_true, theta_pos_true, theta_neg_true,
                             alpha1_true, alpha2_true, k_fourier,
                             gamma_y, gamma_x) {
  ...
  for (i in 1:N) {
    for (tt in 2:TT_total) {
      ...
      fourier_shift <- alpha1_true[i] * sin(2*pi*k_fourier*tt/TT_total) +
                       alpha2_true[i] * cos(2*pi*k_fourier*tt/TT_total)
...
theta_pos_true_val <- 1.0; theta_neg_true_val <- 0.5; k_fourier <- 1
results <- data.frame()
...
for (g in 1:G) {
  ...
  alpha1_true    <- rep(0.3, N)   # sin coefficient
  alpha2_true    <- rep(0.2, N)   # cos coefficient
  ...
  for (r in 1:R) {
    dat <- GenerateDGP_FCS(N, TT, m, rho_f, rho_x, sig_f, sig_u, sig_v,
                            phi_true, theta_pos_true, theta_neg_true,
                            alpha1_true, alpha2_true, k_fourier,
                            gamma_y, gamma_x)
    res <- EstFCS_NARDL(dat$Y, dat$X, N, TT, PT, k_fourier)
  ...
  results <- rbind(results, data.frame(
    N=N, T=TT,
    CS_phi_bias=round(mean(phi_hat)-phi_bar, 4),
    CS_phi_rmse=round(sqrt(mean((phi_hat-phi_bar)^2)), 4),
    CS_phi_pctbias=round(100*(mean(phi_hat)-phi_bar)/abs(phi_bar), 2),
    CS_thpos_bias=round(mean(thp_hat)-theta_pos_true_val, 4),
    CS_thpos_rmse=round(sqrt(mean((thp_hat-theta_pos_true_val)^2)), 4),
    CS_thneg_bias=round(mean(thn_hat)-theta_neg_true_val, 4),
    CS_thneg_rmse=round(sqrt(mean((thn_hat-theta_neg_true_val)^2)), 4)
  ))
  cat(sprintf("phi bias=%.4f (%.1f%%)\n", mean(phi_hat)-phi_bar,
              100*(mean(phi_hat)-phi_bar)/abs(phi_bar)))
}

write.csv(results, "14_mc_fcsnardl_1_CS_results.csv", row.names=FALSE)
cat("\n=== DONE.  Saved: 14_mc_fcsnardl_1_CS_results.csv ===\n")
