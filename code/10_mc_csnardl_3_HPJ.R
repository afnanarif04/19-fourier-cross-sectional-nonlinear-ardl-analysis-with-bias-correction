# =============================================================================
# 10_mc_csnardl_3_HPJ.R
# CS-NARDL Monte Carlo - Half-Panel Jackknife
#
...
# Output   : 10_mc_csnardl_3_HPJ_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" CS-NARDL  |  Script 3 of 6  |  METHOD: HPJ\n")
cat("================================================================\n\n")
  ...
  for (i in 1:N) {
    ...
    for (tt in (PT+2):TT) {
      ...
      for (ell in 0:PT) {
        ...
        XB[row,ell+1] <- xbar[tt-ell]; DB[row,ell+1] <- dxbar[tt-ell]
      }
      row <- row+1
    }
...
EstHPJ_NARDL <- function(Y, X, N, TT, PT) {
  T1 <- floor(TT/2); T2 <- TT-T1
  if (T1 < PT+2 || T2 < PT+2) return(EstCS_NARDL(Y, X, N, TT, PT))
  full <- EstCS_NARDL(Y, X, N, TT, PT)
  h1 <- EstCS_NARDL(Y[,1:T1,drop=F], X[,1:T1,drop=F], N, T1, PT)
  h2 <- EstCS_NARDL(Y[,(T1+1):TT,drop=F], X[,(T1+1):TT,drop=F], N, T2, PT)
  list(phi=2*full$phi-0.5*(h1$phi+h2$phi),
       bp=2*full$bp-0.5*(h1$bp+h2$bp), bn=2*full$bn-0.5*(h1$bn+h2$bn),
       bp1=2*full$bp1-0.5*(h1$bp1+h2$bp1), bn1=2*full$bn1-0.5*(h1$bn1+h2$bn1))
}
...
GenerateDGP_NARDL <- function(N, TT, m, rho_f, rho_x, sig_f, sig_u, sig_v,
  ...
  for (i in 1:N) for (tt in 2:TT_total) {
    ...
    Y[i,tt] <- rho_i*Y[i,tt-1]+beta1_pos*pmax(X[i,tt-1]-X[i,1],0)+
      beta1_neg*pmin(X[i,tt-1]-X[i,1],0)+0.3*(X[i,tt]-X[i,tt-1])+
...
for (g in 1:G) {
  ...
  for (r in 1:R) {
    ...
    res <- EstHPJ_NARDL(dat$Y,dat$X,N,TT,PT)
  ...
  results <- rbind(results, data.frame(
    N=N,T=TT,
    HPJ_phi_bias=round(mean(phi_hat)-phi_bar,4),
    HPJ_phi_rmse=round(sqrt(mean((phi_hat-phi_bar)^2)),4),
    HPJ_phi_pctbias=round(100*(mean(phi_hat)-phi_bar)/abs(phi_bar),2),
    HPJ_thpos_bias=round(mean(thp_hat)-theta_pos_true_val,4),
    HPJ_thpos_rmse=round(sqrt(mean((thp_hat-theta_pos_true_val)^2)),4),
    HPJ_thneg_bias=round(mean(thn_hat)-theta_neg_true_val,4),
    HPJ_thneg_rmse=round(sqrt(mean((thn_hat-theta_neg_true_val)^2)),4)
  ))
  cat(sprintf("phi bias=%.4f\n",mean(phi_hat)-phi_bar))
}
write.csv(results,"10_mc_csnardl_3_HPJ_results.csv",row.names=FALSE)
cat("\n=== DONE.  Saved: 10_mc_csnardl_3_HPJ_results.csv ===\n")
