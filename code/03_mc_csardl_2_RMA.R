# =============================================================================
# 03_mc_csardl_2_RMA.R
# CS-ARDL Monte Carlo - Recursive Mean Adjustment
#
...
# Output   : 03_mc_csardl_2_RMA_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" CS-ARDL  |  Script 2 of 6  |  METHOD: RMA\n")
cat("================================================================\n\n")
...
# ============================================================
# 1.  ESTIMATION: RMA
#     Replace CSA with recursive (expanding-window) means
# ============================================================
EstRMA <- function(Y, X, N, TT, PT) {
  nobs  <- (TT - 1) - PT
  ...
  for (i in 1:N) {
    ...
    for (tt in (PT + 2):TT) {
      ...
      # Recursive means (expanding window up to time tt)
      for (ell in 0:PT) {
        s <- tt - ell
        YB[row, ell + 1] <- mean(Y[, 1:s])
        XB[row, ell + 1] <- mean(X[, 1:s])
        if (s >= 2) {
          DB[row, ell + 1] <- mean(diff(colMeans(X[, 1:s])))
        }
      }
      row <- row + 1
    }
...
GenerateDGP <- function(N, TT, m, rho_f, rho_x, sig_f, sig_u, sig_v,
  ...
  for (i in 1:N) {
    for (tt in 2:TT_total) {
      eps_u <- rnorm(1) * sig_u;  eps_v <- rnorm(1) * sig_v
...
for (g in 1:G) {
  ...
  for (r in 1:R) {
    dat <- GenerateDGP(N, TT, m, rho_f, rho_x, sig_f, sig_u, sig_v,
                       phi_true, beta0_true, beta1_true, gam_true,
                       gamma_y, gamma_x)
    res <- EstRMA(dat$Y, dat$X, N, TT, PT)
  ...
  results <- rbind(results, data.frame(
    N = N, T = TT,
    RMA_phi_bias    = round(bias_phi, 4),
    RMA_phi_rmse    = round(rmse_phi, 4),
    RMA_phi_pctbias = round(pct_phi, 2),
    RMA_theta_bias  = round(bias_th, 4),
    RMA_theta_rmse  = round(rmse_th, 4)
  ))
  cat(sprintf("phi bias=%.4f (%.1f%%)\n", bias_phi, pct_phi))
}

write.csv(results, "03_mc_csardl_2_RMA_results.csv", row.names = FALSE)
cat("\n=== DONE.  Saved: 03_mc_csardl_2_RMA_results.csv ===\n")
