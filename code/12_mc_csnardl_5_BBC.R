# =============================================================================
# 12_mc_csnardl_5_BBC.R
# CS-NARDL Monte Carlo - Bootstrap Bias Correction
#
...
# Output   : 12_mc_csnardl_5_BBC_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" CS-NARDL  |  Script 5 of 6  |  METHOD: BBC (v3)\n")
cat("================================================================\n\n")

set.seed(2026)

EstCS_NARDL_Full <- function(Y, X, N, TT, PT, lambda=1e-6) {
  ybar <- colMeans(Y); xbar <- colMeans(X); dxbar <- c(0,diff(xbar))
  nobs <- (TT-1)-PT
  phi_i <- bp_i <- bn_i <- bp1_i <- bn1_i <- const_i <- numeric(N)
  U <- matrix(0,N,TT)
  for (i in 1:N) {
    yi <- Y[i,]; xi <- X[i,]
    ...
    bp1_i[i] <- b[4]; bn1_i[i] <- b[5]; const_i[i] <- b[7]
    resid <- DY - Z %*% b
    U[i,(PT+2):TT] <- as.numeric(resid)
  }
  list(phi=phi_i,bp=bp_i,bn=bn_i,bp1=bp1_i,bn1=bn1_i,const=const_i,U=U)
}

BBC_NARDL <- function(Y, X, N, TT, PT, B_boot=100, K_iter=3) {
  full <- EstCS_NARDL_Full(Y,X,N,TT,PT)
  phi_curr <- full$phi; bp_curr <- full$bp; bn_curr <- full$bn
  bp1_curr <- full$bp1; bn1_curr <- full$bn1; const_curr <- full$const
  ...
  for (k in 1:K_iter) {
    ...
    for (b in 1:B_boot) {
      ...
      for (i in 1:N) {
        ...
        for (tt in 2:TT) {
          DX_raw <- c(0,diff(X[i,]))
          xp <- cumsum(pmax(DX_raw,0)); xn <- cumsum(pmin(DX_raw,0))
          Y_star[i,tt] <- rho_i*Y_star[i,tt-1]+
            bp_curr[i]*xp[tt]+bn_curr[i]*xn[tt]+
            bp1_curr[i]*xp[max(1,tt-1)]+bn1_curr[i]*xn[max(1,tt-1)]+
            0.3*(X[i,tt]-X[i,tt-1])+const_curr[i]+full$U[i,tt]*eta[tt]
        }
      }
      res_b <- tryCatch(EstCS_NARDL_Full(Y_star,X,N,TT,PT,lambda=1e-5),error=function(e)NULL)
  ...
  theta_pos_i <- -(bp_curr+bp1_curr)/phi_curr
  theta_neg_i <- -(bn_curr+bn1_curr)/phi_curr
  w <- phi_curr^2/sum(phi_curr^2)
  list(phi=phi_curr, theta_pos=sum(w*theta_pos_i), theta_neg=sum(w*theta_neg_i))
}
...
write.csv(results,"12_mc_csnardl_5_BBC_results.csv",row.names=FALSE)
cat("\n=== DONE.  Saved: 12_mc_csnardl_5_BBC_results.csv ===\n")
