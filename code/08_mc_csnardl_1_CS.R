# =============================================================================
# 08_mc_csnardl_1_CS.R
# CS-NARDL Monte Carlo - Uncorrected CCEMG (baseline)
#
# Method   : CS: Uncorrected common correlated effects mean group
# Reference : Pesaran (2006); Chudik and Pesaran (2015); Shin et al. (2014)
#
...
# Output   : 08_mc_csnardl_1_CS_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" CS-NARDL  |  Script 1 of 6  |  METHOD: Baseline\n")
cat("================================================================\n\n")

set.seed(2026)

# ============================================================
# 1.  CS-NARDL ESTIMATOR
# ============================================================
...
# ============================================================
# 2.  DGP (NARDL: asymmetric long-run)
# ============================================================
GenerateDGP_NARDL <- function(N, TT, m, rho_f, rho_x, sig_f, sig_u, sig_v,
  ...
  for (i in 1:N) {
    for (tt in 2:TT_total) {
      ...
      DX <- X[i, tt] - X[i, tt - 1]
      DX_pos <- max(DX, 0);  DX_neg <- min(DX, 0)

      rho_i <- 1 + phi_true[i]
      # Asymmetric levels effect via partial sums
...
for (g in 1:G) {
  ...
  rmse_thp <- sqrt(mean((thp_hat - theta_pos_true_val)^2))
  bias_thn <- mean(thn_hat) - theta_neg_true_val
  rmse_thn <- sqrt(mean((thn_hat - theta_neg_true_val)^2))

  results <- rbind(results, data.frame(
    N=N, T=TT,
    CS_phi_bias=round(bias_phi,4), CS_phi_rmse=round(rmse_phi,4),
    CS_phi_pctbias=round(pct_phi,2),
    CS_thpos_bias=round(bias_thp,4), CS_thpos_rmse=round(rmse_thp,4),
    CS_thneg_bias=round(bias_thn,4), CS_thneg_rmse=round(rmse_thn,4)
  ))
  cat(sprintf("phi bias=%.4f (%.1f%%)\n", bias_phi, pct_phi))
}

write.csv(results, "08_mc_csnardl_1_CS_results.csv", row.names=FALSE)
cat("\n=== DONE.  Saved: 08_mc_csnardl_1_CS_results.csv ===\n")
