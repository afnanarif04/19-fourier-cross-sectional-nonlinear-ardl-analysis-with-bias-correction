# =============================================================================
# 02_mc_csardl_1_CS.R
# CS-ARDL Monte Carlo - Uncorrected CCEMG (baseline)
#
# Method   : CS: Uncorrected common correlated effects mean group
# Reference : Pesaran (2006); Chudik and Pesaran (2015)
#
...
# Output   : 02_mc_csardl_1_CS_results.csv
#
...
rm(list = ls())
cat("================================================================\n")
cat(" CS-ARDL  |  Script 1 of 6  |  METHOD: CS (Baseline)\n")
cat("================================================================\n\n")

set.seed(2026)

# ============================================================
# 1.  ESTIMATION FUNCTION: CS (uncorrected)
# ============================================================
EstCS <- function(Y, X, N, TT, PT) {
  ...
  for (i in 1:N) {
    ...
    phi_i[i] <- b[1];  b0_i[i] <- b[2]
    b1_i[i]  <- b[3];  gam_i[i] <- b[4]
  }
  list(phi = phi_i, b0 = b0_i, b1 = b1_i, gam = gam_i)
}

# ============================================================
# 2.  DGP  (CP 2015, corrected)
# ============================================================
  ...
  for (i in 1:N) {
    for (tt in 2:TT_total) {
      ...
      X[i, tt] <- rho_x * X[i, tt - 1] +
                  sum(gamma_x[i, ] * f[, tt]) + eps_v
      rho_i <- 1 + phi_true[i]
      Y[i, tt] <- rho_i * Y[i, tt - 1] +
                  beta0_true[i] * X[i, tt] +
                  beta1_true[i] * X[i, tt - 1] +
                  gam_true[i] * (X[i, tt] - X[i, tt - 1]) +
                  sum(gamma_y[i, ] * f[, tt]) + eps_u
    }
  }
  list(Y = Y[, (Tburn + 1):TT_total],
       X = X[, (Tburn + 1):TT_total])
}

# ============================================================
# 3.  GRID & TRUE PARAMETERS
# ============================================================
...
m      <- 1
rho_f  <- 0.6;   rho_x <- 0.6
sig_f  <- 1.0;   sig_u <- 1.0;  sig_v <- 1.0
theta_true <- 1.0                           # long-run coefficient
...
for (g in 1:G) {
  ...
  beta1_true <- phi_true * (-theta_true)      # beta1 = -phi*theta
  beta0_true <- rep(0.5, N)
  gam_true   <- rep(0.3, N)
  ...
  cat(sprintf("Grid %2d/%d  (N=%3d, T=%3d, PT=%d) ... ", g, G, N, TT, PT))

  for (r in 1:R) {
    dat  <- GenerateDGP(N, TT, m, rho_f, rho_x, sig_f, sig_u, sig_v,
                        phi_true, beta0_true, beta1_true, gam_true,
                        gamma_y, gamma_x)
    res  <- EstCS(dat$Y, dat$X, N, TT, PT)
  ...
  results <- rbind(results, data.frame(
    N = N, T = TT,
    CS_phi_bias   = round(bias_phi, 4),
    CS_phi_rmse   = round(rmse_phi, 4),
    CS_phi_pctbias = round(pct_phi, 2),
    CS_theta_bias = round(bias_th, 4),
    CS_theta_rmse = round(rmse_th, 4)
  ))
  cat(sprintf("phi bias=%.4f (%.1f%%)  theta bias=%.4f\n",
              bias_phi, pct_phi, bias_th))
}

# ============================================================
# 5.  SAVE
# ============================================================
write.csv(results, "02_mc_csardl_1_CS_results.csv", row.names = FALSE)
cat("\n=== DONE.  Saved: 02_mc_csardl_1_CS_results.csv ===\n")
